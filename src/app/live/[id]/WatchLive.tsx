...
const Live = ({ params: { id: liveId } }: { params: { id: string } }) => {
  ...
  const [livePeerId, setLivePeerId] = useState<string>('');
  ...
  function joinLive(): void | NodeJS.Timeout {
    if (!ws || !ws.readyState) {
      return setTimeout(joinLive, 1000);
    }
  ...
  useEffect(() => {
    ...
    emitter.on(socket_events.JOIN_LIVE, (data) => {
      setLivePeerId(data.hostPeerId);
    });
    ...
    return () => {
      ...
      emitter.off(socket_events.JOIN_LIVE, () => {});
    };
  }, [emitter]);
  ...
  function callPeer() {
    const call = peerRef.current?.call(liveId, new MediaStream());
    if (!call) {
      return console.error('failed to get Peer call');
    }
    ...
    <>
      ...
      <section className="absolute top-16 z-10 w-full">
        <div className="flex h-full w-full justify-between py-2 pl-4 pr-2">
          <button
            onClick={() => callPeer()}
            className="rounded-full  bg-slate-800 px-1"
          >
